...
if(isset($_POST['submit'])){
  ...
  $added_on=date('Y-m-d h:i:s');
    if(mysqli_num_rows(mysqli_query($conn,"SELECT * FROM `user` WHERE name= '$name'"))>0){
              ...
              $sql="SELECT * FROM `user` WHERE name='$name' and id!='$id'";
                ...
                if(mysqli_num_rows(mysqli_query($conn,$sql))>0){
                  $msg="User already added";
                    ...
                    if($id==''){
                      mysqli_query($conn,"insert into user (name,email,mobile,password,status,added_on) value('$name','$email','$mobile','$password',1,'$added_on')");
                        redirect('user.php');
                      }else{
                        mysqli_query($conn,"update user set name='$name',email='$email',mobile='$mobile',password='$password' where id='$id'");
                        redirect('user.php');
                      }
                }
    }
                    ...
                    <div class="modal-body">
                      <form method="post">
                        ...
                        <div class="form-group">
                          ...
                          <input type="email" value="<?=$result['email'];?>" class="form-control" name="email" id="recipient-name">
                        </div>
                        <div class="form-group">
